...
class HoughComputation(object):
    ...
    def getHoughLines(self,heatmap,threshold,line_length,line_gap,filter_percent):
        ...
        lines = probabilistic_hough_line(heatmap_matrix,threshold=threshold,line_length=line_length, line_gap=line_gap, theta=None, seed=42)
    ...
    def plotHoughLines(self, houghlines, image, save=False, fname=None):
        ...
        if image.shape[1] > image.shape[0]:
            fig, axes = plt.subplots(2,1,figsize=(20,20), sharex=False, sharey=False)
        elif image.shape[0] > image.shape[1]:
            fig, axes = plt.subplots(1,2,figsize=(20,20), sharex=False, sharey=False)
        ...
        ax[1].imshow(invert(np.sqrt(np.sqrt(image))) * 0, cmap=cm.gray_r)
