...
def to_file(label,names,sizes=None):
    ...
    for c,name in enumerate(names):
        ...
        for l in label[curr_ind:curr_ind+sizes[c]]:
            f.write(str(l)+"\n")
        f.close()
def calc_mapping(cluster1,centroid2,save=True):
...
if __name__ == "__main__":
    ...
    # names = ["standard heatmap","standard centroids",name2+" heatmap",name2+" centroids"]
    # print(tot.shape)
    # print(sizes)
    # t_sne(tot,sizes,colors,names=names,dist='euclidean')
    ...
    hg = HeatmapGenerator()
    cl = Clustering()
    map_centroids = calc_mapping(cluster1,centroid2)[0]
    ...
    for i in set(label2):
        ...
        hg.compareHeatmaps(a**4,b**4,"cluster-"+str(map_centroids[i]),True,"cluster-"+str(i),titles=("standard","mkdir"),metric=metric)
