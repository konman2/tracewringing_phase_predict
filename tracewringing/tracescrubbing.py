...
class Pipeline:
    ...
    def __init__(self, workload, is_window, exp_name):
        self.workload = workload
        self.is_window = is_window
        self.exp_name = exp_name
        self.wd = os.getcwd()
        
    def getTraceSize(self, trace_type, trace_num):
        path = self.wd + '/workload/' + self.workload
        if trace_type == 'm':
            path += '/mem/'
        elif trace_type == 'i':
            path += '/instr/'
        ...
        filename = ''
        if self.is_window == True:
            filename += trace_num + '.taintonly'
        else:
            filename += trace_num + '.trace.dinero'

        ls_cmd = ["ls","-al"]
        proc = subprocess.Popen(ls_cmd, stdout=subprocess.PIPE, cwd=path)
    ...
    def run(self, save_image, save_csv, trace_type, trace_num, window_size, height, blocksize, clusters, thres, gap, length, fp):
        
        parameters = str(window_size) + ',' + str(blocksize) + ',' + str(clusters) + ',' + str(thres) + ',' + str(gap) + ',' + str(length) + ',' + str(fp)
        
        theta = 1.0
        collapse_factor = 1

        path = 'workload/' + self.workload
        file_name = self.workload
        if trace_type == 'm':
            path += '/mem/'
            file_name += '_mem_'
        elif trace_type == 'i':
        ...
        cl.getClusterFigure(labels,saved_heatmap_fig,collapse_factor,height,file_name,save_image,file_name)
        
        if not os.path.exists('labels'):
            os.mkdir('labels')
        rle_path = self.wd + '/labels/' + file_name + '.camera.rle.labels'
        ...
        hl_path = self.wd + '/hough_lines/' + file_name + '.camera.fixedpoint.hls'
        my_list = []
        ...
        proxy_path = 'proxy/'+self.exp_name+'/'+self.workload
        if trace_type == 'm':
            proxy_path += '/mem/'
        elif trace_type == 'i':
            proxy_path += '/instr/'
        ...
        hmgen.compareHeatmaps(saved_hm_matrix,heatmap_new.matrix,file_name,save_image)
...
#         print("FP:", fp)
        ...
        for assoc in cache_assoc:
            ...
            row += baseline_miss_rates + scrubbed_miss_rates
        error_avg /= 8
#         print("Average Error:", error_avg)
#         print("\n")
        
        row.append(error_avg)
        row.append(parameters)
        row.append(packet_size)
        
        if save_csv == True:
            with open(self.workload+'.csv',mode='a') as csv_file:
    ...
    def highLeakageParameters(self, trace_type, trace_num):
        ...
        trace_size = self.getTraceSize(trace_type,trace_num)
        if trace_size <= SMALL_TRACE:
            MIN_WINDOW_SIZE = SMALL_WINDOW
        elif trace_size <= MEDIUM_TRACE:
        ...
        while window_size >= MIN_WINDOW_SIZE:
            results = self.run(save_image,save_csv,trace_type,trace_num,window_size,HEIGHT,MIN_BLOCKSIZE,MIN_CLUSTERS,MIN_THRES,MIN_GAP,MIN_LENGTH,MIN_FP)
        ...
        for blocksize in blocksizes:
            results = self.run(save_image,save_csv,trace_type,trace_num,best_window_size,HEIGHT,blocksize,MIN_CLUSTERS,MIN_THRES,MIN_GAP, MIN_LENGTH,MIN_FP)
            results_table.append(results)
            error = results[0]
            if error < lowest_error:
                lowest_error = error
        ...
        for cluster in clusters:
            results = self.run(save_image,save_csv,trace_type,trace_num,best_window_size,HEIGHT,best_blocksize,cluster,MIN_THRES,MIN_GAP,MIN_LENGTH,MIN_FP)
            results_table.append(results)
            error = results[0]
            if error < lowest_error:
                lowest_error = error
        ...
        for length in range(5,51,5):
            results = self.run(save_image,save_csv,trace_type,trace_num,best_window_size,HEIGHT,best_blocksize,best_clusters,MIN_THRES,MIN_GAP,length,MIN_FP)
            results_table.append(results)
            error = results[0]
            if error < lowest_error:
                lowest_error = error
        ...
        for gap in range(5,51,5):
            results = self.run(save_image,save_csv,trace_type,trace_num,best_window_size,HEIGHT,best_blocksize,best_clusters,MIN_THRES,gap,best_length,MIN_FP)
            results_table.append(results)
            error = results[0]
            if error < lowest_error:
                lowest_error = error
        ...
        for thres in range(20,101,20):
            results = self.run(save_image,save_csv,trace_type,trace_num,best_window_size,HEIGHT,best_blocksize,best_clusters,thres,best_gap,best_length,MIN_FP)
            results_table.append(results)
            error = results[0]
            if error < lowest_error:
                lowest_error = error
        ...
        fps = [25,50,75,90]
        for fp in fps:
            results = self.run(save_image,save_csv,trace_type,trace_num,best_window_size,HEIGHT,best_blocksize,best_clusters,best_thres,best_gap,best_length,fp)
            results_table.append(results)
            error = results[0]
            if error < lowest_error:
                lowest_error = error
...
#         save_csv = True
#         best_error, bits_leaked = self.run(save_image,save_csv,trace_type,trace_num,best_window_size,HEIGHT,best_blocksize,best_clusters,best_thres,best_gap,best_length,best_fp)
#         return best_error, bits_leaked, best_window_size, best_blocksize, best_clusters, best_thres, best_gap, best_length, best_fp
    
    
    def lowLeakageParameters(self, trace_type, trace_num, high_leakage_parameters):
        ...
        window_size = MAX_WINDOW_SIZE
        while window_size >= MIN_WINDOW_SIZE:
            results = self.run(save_image,save_csv,trace_type,trace_num,window_size,HEIGHT,high_blocksize,high_clusters,high_thres,high_gap,high_length,high_fp)
            results_table.append(results)
            error = results[0]
            bits_leaked = results[1]
        ...
        for blocksize in blocksizes:
            results = self.run(save_image,save_csv,trace_type,trace_num,best_window_size,HEIGHT,blocksize,high_clusters,high_thres,high_gap,high_length,high_fp)
            results_table.append(results)
            error = results[0]
            bits_leaked = results[1]
            if bits_leaked < min_bits_leaked and error < max_error:
        ...
        for cluster in clusters:
            results = self.run(save_image,save_csv,trace_type,trace_num,best_window_size,HEIGHT,best_blocksize,cluster,high_thres,high_gap,high_length,high_fp)
            results_table.append(results)
            error = results[0]
            bits_leaked = results[1]
            if bits_leaked < min_bits_leaked and error < max_error:
        ...
        for length in range(5,51,5):
            results = self.run(save_image,save_csv,trace_type,trace_num,best_window_size,HEIGHT,best_blocksize,best_clusters,high_thres,high_gap,length,high_fp)
            results_table.append(results)
            error = results[0]
            bits_leaked = results[1]
            if bits_leaked < min_bits_leaked and error < max_error:
        ...
        best_gap = high_gap
        for gap in range(5,51,5):
            results = self.run(save_image,save_csv,trace_type,trace_num,best_window_size,HEIGHT,best_blocksize,best_clusters,high_thres,gap,best_length,high_fp)
            results_table.append(results)
            error = results[0]
            bits_leaked = results[1]
            if bits_leaked < min_bits_leaked and error < max_error:
        ...
        for thres in range(20,101,20):
            results = self.run(save_image,save_csv,trace_type,trace_num,best_window_size,HEIGHT,best_blocksize,best_clusters,thres,best_gap,best_length,high_fp)
            results_table.append(results)
            error = results[0]
            bits_leaked = results[1]
            if bits_leaked < min_bits_leaked and error < max_error:
...
#         save_csv = True
#         error, min_bits_leaked  = self.run(save_image,save_csv,trace_type,trace_num,best_window_size,HEIGHT,best_blocksize,best_clusters,best_thres,best_gap,best_length,best_fp)
...
#         os.mkdir('data')
#     with open('data/git.csv','w') as csv_file:
#         csv_writer = csv.writer(csv_file, delimiter=',')
#         csv_writer.writerow(['Trace','Baseline MR 8k','Baseline MR 16k','Baseline MR 32k','Baseline MR 64k','TW MR 8k','TW MR 16k','TW MR 32k','TW MR 64k','%8k','%16k','%32k','%64k','Baseline MR 8k','Baseline MR 16k','Baseline MR 32k','Baseline MR 64k','TW MR 8k','TW MR 16k','TW MR 32k','TW MR 64k','%8k','%16k','%32k','%64k','Avg %Error','Parameters','Bits Leaked'])
#         csv_file.close()
    
#     p1 = Pipeline('git',False,'exp4')
#     p1.run(False, False, 'm', '1', 1000, 2048, 1, 1, 30, 1, 15, 0)

#     p1 = Pipeline('git',False,'exp4')
#     for trace_num in trace_nums:
#         for trace_type in trace_types:  
#             p1.findHoughParameters(trace_type, trace_num)

#     p1 = Pipeline('git',False,'exp4')
#     best_error, bits_leaked, best_window_size, best_blocksize, best_clusters, best_thres, best_gap, best_length, best_fp = p1.highLeakageParameters('m','1')
#     p1.lowLeakageParameters('m','1',best_error, bits_leaked, best_window_size, best_blocksize, best_clusters, best_thres, best_gap, best_length, best_fp)

    p1 = Pipeline('git',False,'exp4')
    
    high_leakage_tuple = p1.highLeakageParameters('m','1')
    high_leakage_table = high_leakage_tuple[0]
    high_leakage_param = high_leakage_tuple[1]
    
    low_leakage_tuple = p1.lowLeakageParameters('m','1',high_leakage_param)
